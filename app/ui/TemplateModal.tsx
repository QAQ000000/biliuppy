...
const TemplateModal: React.FC<TemplateModalProps> = ({ children, entity, onOk }) => {
  ...
  const initialValues = useMemo(() => {
    if (!entity || typeof entity.time_range !== 'string') return entity
    try {
      const timeRange: string[] = JSON.parse(entity.time_range)
      return { ...entity, time_range: timeRange.map(value => new Date(value)) }
    } catch {
      return entity
    }
  }, [entity])
  ...
  return (
    <>
      ...
      >
        <Form initValues={initialValues} getFormApi={formApi => (api.current = formApi)}>
          <Form.Input
            field="remark"
            label="录播备注"
            trigger="blur"
          ...
          <Form.Input
            ...
            placeholder="flv"
            style={{ width: 176 }}
            helpText="视频保存格式。Youtube 平台可能由源格式决定。"
          />
