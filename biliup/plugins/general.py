...
class YDownload(DownloadBase):
    ...
    def get_sinfo(self):
        ...
        with yt_dlp.YoutubeDL() as ydl:
            ...
            self.raw_stream_url = info.get('url')
            for i in info['formats']:
                info_list.append(i['format_id'])
                if i.get('url'):
                    self.raw_stream_url = i['url']
    ...
    async def acheck_stream(self, is_check=False):
        ...
        try:
            ...
            if streams:
                ...
                self.raw_stream_url = self.stream.to_url()
                fd = self.stream.open()
                fd.close()
                # streams.close()
                return True
        ...
        # print(info.title)
            ...
            for handler in handlers:
                if await handler.acheck_stream():
                    ...
                    self.raw_stream_url = handler.raw_stream_url
                    self.room_title = handler.room_title
                    self.stream_headers = handler.stream_headers
                    return True
            return False
        return True
