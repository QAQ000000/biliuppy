...
class BiliWeb(UploadBase):
    def __init__(
        ...
        self.up_selection_reply = up_selection_reply
        self.up_close_reply = up_close_reply
    ...
    def upload(
        ...
        with BiliBili(video) as bili:
            ...
            video.dolby = self.dolby
            video.hires = self.hires
            video.no_reprint = self.no_reprint
            video.is_only_self = self.is_only_self
            video.charging_pay = self.charging_pay
            video.up_selection_reply = self.up_selection_reply
            video.up_close_reply = self.up_close_reply
            video.up_close_danmu = self.up_close_danmu
            video.extra_fields = self.extra_fields
            if self.dtime:
...
class BiliBili:
    ...
    def cookie_dict(cookies):
        if "cookie_info" in cookies:
            return {item["name"]: item["value"] for item in cookies["cookie_info"].get("cookies", [])}
        if isinstance(cookies.get("cookies"), list):
